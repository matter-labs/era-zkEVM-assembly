...
pub struct Div {
    ...
    /// The quotient destination register.
    pub quotient_destination: FullOperand,
    /// The remainder destination register.
    pub remainder_destination: RegisterOperand,
    /// if it is set then source operands have be swapped.
    ...
    pub fn build_from_parts(
        ...
        if !modifiers.is_empty() {
            return Err(InstructionReadError::UnknownArgument(format!(
                "Div instruction contains unknown modifiers: {:?}",
                modifiers
            )));
        }
        ...
        let new = Self {
            ...
            quotient_destination: dst0,
            remainder_destination: dst1.as_register_operand(3)?,
            swap_operands,
        };
...
impl<const N: usize, E: VmEncodingMode<N>> TryFrom<Div> for DecodedOpcode<N, E> {
    ...
    fn try_from(value: Div) -> Result<Self, Self::Error> {
        let mut new = DecodedOpcode::default();
        new.variant = OpcodeVariant {
            opcode: Opcode::Div(DivOpcode),
        ...
        set_src0_or_dst0_full_operand(
            &value.quotient_destination.as_generic_operand(2)?,
            &mut new,
            true,
        );
        set_register_operand(&value.remainder_destination, &mut new, true);
